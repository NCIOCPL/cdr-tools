...
class Trial:
    def __init__(self, row):
        ...
        self.ctepPersonId = unicode(str(row[4]), 'utf-8').strip()
        self.specificPhone = unicode(str(row[6]), 'utf-8').strip()
        self.specificEmail = unicode(str(row[7]), 'utf-8').strip()
        self.personName = unicode(str(row[5]), 'utf-8').strip()
...
"""
    def run(self, docObj):
        ...
        link = u"CDR%010d#%s" % (trial.cdrPersonId, trial.fragmentId)
...
if len(sys.argv) < 4 or sys.argv[3] not in ('test', 'live'):
    ...
    sys.exit(1)
xlsFile = "CTEP TRIALS WITH RESPONSIBLE PARTY 2009_04_08.xls"
book    = ExcelReader.Workbook(xlsFile)
sheet   = book[0]
trials  = {}
cursor  = cdrdb.connect('CdrGuest').cursor()
unmappedCtepPersonIds = set()
for row in sheet:
    try:
        trial = Trial(row)
        ...
        rows = cursor.fetchall()
        if not rows:
            unmappedCtepPersonIds.add(trial.ctepPersonId)
        ...
        if trial.cdrPersonId in coopGroupMap:
            trial.fragmentId = coopGroupMap[trial.cdrPersonId]
        else:
            cursor.execute("""\
                SELECT value
                  FROM query_term
...
print "UNMAPPED CTEP PERSON IDS:"
for personId in unmappedCtepPersonIds:
    print personId
...
job = ModifyDocs.Job(sys.argv[1], sys.argv[2], filterObject, transformObject,
                     "Adding ResponsibleParty to CTEP trials (request #4547).",
