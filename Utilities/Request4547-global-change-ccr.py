...
class Trial:
    def __init__(self, row):
        ...
        self.cdrPersonId = persons.get(self.cdrId)
        self.specificPhone = unicode(str(row[5]), 'utf-8').strip()
        self.specificEmail = unicode(str(row[6]), 'utf-8').strip()
        self.personName = unicode(str(row[4]), 'utf-8').strip()
...
if len(sys.argv) < 4 or sys.argv[3] not in ('test', 'live'):
    ...
    sys.exit(1)
xlsFile = 'CcrTrialsWithResponsiblePartyInformation2.xls'
xlsFile = 'Request4547-final-ccr-data.xls'
book    = ExcelReader.Workbook(xlsFile)
sheet   = book[0]
trials  = {}
cursor  = cdrdb.connect('CdrGuest').cursor()
persons = {}
for line in open('Request4547-ccr-mapping.txt'):
    try:
        trialId, name, personId = line.strip().split('\t')
        persons[int(trialId)] = int(personId)
    except:
        pass
print "loaded %d person IDs" % len(persons)
for row in sheet:
    try:
        trial = Trial(row)
        if not trial.cdrPersonId:
            msg = "trial %d has no Person ID" % trial.cdrId
...
job = ModifyDocs.Job(sys.argv[1], sys.argv[2], filterObject, transformObject,
                     "Adding ResponsibleParty to CCR trials (request #4547).",
