#----------------------------------------------------------------------
#
# $Id: CC_Closed_AddPUP0.py,v 1.1 2009-09-25 19:09:49 venglisc Exp $
#
...
class Protocol:
    def __init__(self, protInfo, id, cursor):
        ...
        protInfo[u'pupId']   = self.pup.pupId
        protInfo[u'fName']   = self.pup.fName
        protInfo[u'lName']   = self.pup.lName
        protInfo[u'contact'] = self.pup.contact
        protInfo[u'phone']   = self.pup.phone
        protInfo[u'email']   = self.pup.email
...
def readMaster(iSheet = 3, filename = 'd:/cdr/tmp/CCR_Protocols.xls'):
    ...
    for row in sheet.rows:
        ...
        #if rownum > 500: return Protocols
    return Protocols
...
for key in wsTitle.keys():
    ...
    if key == 'mst':
        ...
        exRow.addCell(8, 'PUP')
        exRow.addCell(9, 'PUP Phone')
        exRow.addCell(10, 'PUP Email')
        exRow.addCell(11, 'PUP ID')
    ...
    if key == 'mst':
        ...
        for row in Protocols.keys():
            ...
            if Protocols[row].has_key(u'lName') and \
               Protocols[row][u'lName']:
                exRow.addCell(8, Protocols[row][u'lName'] + ", " + 
                                 Protocols[row][u'fName'])
            if Protocols[row].has_key(u'phone') and \
               Protocols[row][u'phone']:
                exRow.addCell(9, Protocols[row][u'phone'])
            if Protocols[row].has_key(u'email') and \
               Protocols[row][u'email']:
                exRow.addCell(10, Protocols[row][u'email'])
